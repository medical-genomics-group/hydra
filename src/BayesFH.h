...
#ifndef SRC_BAYESFH_H_
#define SRC_BAYESFH_H_
#include "data.hpp"
...
public:
  ...
  const bool use_xfiles_in_restart =
      true; // Restart from .xbet rather than .bet file
  ...
  VectorXd denom; // temporal variable for computing the inflation of the effect
                  // variance for a given non-zero componnet
  VectorXi m0;    // total number of markers in model

  // EO@@@ watch type change by EO for cass (former v)
  // DT@@@ there is no side effects for letting cass be a matrix. Just change
  // indexing in Bw
  MatrixXi cass;
  MatrixXi cass8;
  ...
  VectorXd y_tilde; // variable containing the adjusted residuals to exclude the
                    // effects of a given marker
  VectorXd epsilon; // variable containing the residuals
  VectorXi components;
  MatrixXi components8;

  VectorXd epsilon_restart; // EO:to store epsilon found in .eps* file when
                            // restarting a chain
  ...
  BayesFH(Data &data, Options &opt, const long memPageSize);
  virtual ~BayesFH();
  ...
  void offset_vector_f64(double *__restrict__ vec, const double offset,
                         const int N);

  void set_vector_f64(double *__restrict__ vec, const double val, const int N);

  void sum_vectors_f64(double *__restrict__ out, const double *__restrict__ in1,
                       const double *__restrict__ in2, const int N);

  void sum_vectors_f64(double *__restrict__ out, const double *__restrict__ in1,
                       const int N);

  double sum_vector_elements_f64_base(const double *__restrict__ vec,
                                      const int N);

  double sum_vector_elements_f64(const double *__restrict__ vec, const int N);

  void copy_vector_f64(double *__restrict__ dest,
                       const double *__restrict__ source, const int N);

  void sparse_add(double *__restrict__ vec, const double val,
                  const uint *__restrict__ IX, const size_t NXS,
                  const size_t NXL);

  void sparse_scaadd(double *__restrict__ vout, const double dMULT,
                     ...
                     const size_t NML, const double mu, const double sig_inv,
                     const int N);
  ...
  double sparse_dotprod(const double *__restrict__ vin1,
                        ...
                        const uint *__restrict__ IM, const size_t NMS,
                        const size_t NML, const double mu, const double sig_inv,
                        const int N, const int marker);

  int runGibbs();

#ifdef USE_MPI
  void init_from_restart(const int K, const uint M, const uint Mtot,
                         const uint Ntotc, const int *MrankS, const int *MrankL,
                         const bool use_xbet);
  ...
  void check_whole_array_was_set(const uint *array, const size_t size,
                                 const int linenumber, const char *filename);

  void mpi_assign_blocks_to_tasks(const uint numBlocks,
                                  const vector<int> blocksStarts,
                                  const vector<int> blocksEnds, const uint Mtot,
                                  const int nranks, const int rank, int *MrankS,
