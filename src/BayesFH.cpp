...
#endif
BayesFH::BayesFH(Data &data, Options &opt, const long memPageSize)
    : data(data), opt(opt), bedFile(opt.bedFile + ".bed"),
      memPageSize(memPageSize), seed(opt.seed), max_iterations(opt.chainLength),
      burn_in(opt.burnin)
      //, dist(opt.seed)
      ,
      usePreprocessedData(opt.analysisType == "PPBayes"), showDebug(false) {
  double *ptr = &opt.S[0];
  cva = (Eigen::Map<Eigen::VectorXd>(ptr, static_cast<long>(opt.S.size())))
            .cast<double>();
}

BayesFH::~BayesFH() {}

void BayesFH::offset_vector_f64(double *__restrict__ vec, const double offset,
                                 const int N) {
...
void BayesFH::set_vector_f64(double *__restrict__ vec, const double val,
                              const int N) {
...
/*
  inline void copy_vector_f64(double* __restrict__ dest, const double*
  __restrict__ source, const int N) {
...
void BayesFH::copy_vector_f64(double *__restrict__ dest,
                               const double *__restrict__ source, const int N) {
#ifdef __INTEL_COMPILER
  __assume_aligned(dest, 64);
  __assume_aligned(source, 64);
#endif
...
double BayesFH::sum_vector_elements_f64_base(const double *__restrict__ vec,
                                              const int N) {
...
double BayesFH::sum_vector_elements_f64(const double *__restrict__ vec,
                                         const int N) {
...
void BayesFH::sum_vectors_f64(double *__restrict__ out,
                               const double *__restrict__ in1,
                               const double *__restrict__ in2, const int N) {
#ifdef __INTEL_COMPILER
...
/*
  inline void sum_vectors_f64(double* __restrict__ out, const double*
  __restrict__ in1, const double* __restrict__ in2, const int N) {
...
void BayesFH::sum_vectors_f64(double *__restrict__ out,
                               const double *__restrict__ in1, const int N) {
...
inline void sum_vectors_f64_ref(double *__restrict__ out,
                                const double *__restrict__ in1,
                                const double *__restrict__ in2, const int N) {
#ifdef __INTEL_COMPILER
  __assume_aligned(in1, 64);
...
inline void scaadd(double *__restrict__ vout, const double *__restrict__ vin1,
                   const double *__restrict__ vin2, const double dMULT,
                   const int N) {
...
inline void sparse_set(double *__restrict__ vec, const double val,
                       const uint *__restrict__ IX, const size_t NXS,
                       const size_t NXL) {
#ifdef __INTEL_COMPILER
  __assume_aligned(vec, 64);
...
// inline
void BayesFH::sparse_add(double *__restrict__ vec, const double val,
                          const uint *__restrict__ IX, const size_t NXS,
                          const size_t NXL) {
...
void BayesFH::sparse_scaadd(double *__restrict__ vout, const double dMULT,
                             const uint *__restrict__ I1, const size_t N1S,
                             const size_t N1L, const uint *__restrict__ I2,
                             const size_t N2S, const size_t N2L,
                             const uint *__restrict__ IM, const size_t NMS,
                             const size_t NML, const double mu,
                             const double sig_inv, const int N) {
  ...
  if (dMULT == 0.0) {
    ...
    // printf("sparse_scaadd aux = %15.10f with mu = %15.10f, dbetsig =
...
inline double partial_sparse_dotprod(const double *__restrict__ vec,
                                     ...
                                     const size_t NXS, const size_t NXL,
                                     const double fac) {
  ...
  //}
  // t1 += mysecond();
  // printf("kerold 1 BW = %g\n", double(N1L)*sizeof(double) / 1024. / 1024. /
  // t1);
...
double BayesFH::sparse_dotprod(const double *__restrict__ vin1,
                                const uint *__restrict__ I1, const size_t N1S,
                                const size_t N1L, const uint *__restrict__ I2,
                                const size_t N2S, const size_t N2L,
                                const uint *__restrict__ IM, const size_t NMS,
...
inline void scaadd(double *__restrict__ vout, const double *__restrict__ vin,
                   const double dMULT, const int N) {
...
inline double dotprod(const double *__restrict__ vec1,
                      const double *__restrict__ vec2, const int N) {
...
// -----------------------------------------------------
void BayesFH::mpi_define_blocks_of_markers(const int Mtot, int *MrankS,
                                            int *MrankL, const uint nblocks) {
...
// -------------------------------------------------------------------------
void BayesFH::check_whole_array_was_set(const uint *array, const size_t size,
...
//      1) Compute the number of ones and twos to be written by each task to
//      compute
//         rank-wise offsets for writing the si1 and si2 files
//      2) Write files with global indexing
// ---------------------------------------------------------------------------------
void BayesFH::write_sparse_data_files(const uint bpr) {

  int rank, nranks, result;

  MPI_Comm_size(MPI_COMM_WORLD, &nranks);
  MPI_Comm_rank(MPI_COMM_WORLD, &rank);
  MPI_File bedfh, dimfh, si1fh, sl1fh, ss1fh, si2fh, sl2fh, ss2fh, simfh, slmfh,
      ssmfh;
  MPI_Offset offset;
  MPI_Status status;
  ...
  if (rank == 0)
    printf("INFO   : will generate sparse data files out of %d ranks and %d "
           "blocks per rank.\n",
           nranks, bpr);
  ...
  if (rank == 0)
    printf("INFO   : full dataset includes %d markers and %d individuals.\n",
           Mtot, N);
  ...
  const size_t snpLenByt =
      (data.numInds % 4) ? data.numInds / 4 + 1 : data.numInds / 4;
  if (rank == 0)
    printf("INFO   : snpLenByt = %zu bytes.\n", snpLenByt);

  // Get bed file directory and basename
  std::string sparseOut = mpi_get_sparse_output_filebase(rank);
  if (rank == 0)
    printf("INFO   : will write sparse output files as: %s.{ss1, sl1, si1, "
           "ss2, sl2, si2}\n",
           sparseOut.c_str());
  ...
  bedfp += ".bed";
  check_mpi(MPI_File_open(MPI_COMM_WORLD, bedfp.c_str(), MPI_MODE_RDONLY,
                          MPI_INFO_NULL, &bedfh),
            __LINE__, __FILE__);
  ...
  check_mpi(MPI_File_open(MPI_COMM_WORLD, dim.c_str(),
                          MPI_MODE_CREATE | MPI_MODE_WRONLY | MPI_MODE_EXCL,
                          MPI_INFO_NULL, &dimfh),
            __LINE__, __FILE__);
  check_mpi(MPI_File_open(MPI_COMM_WORLD, si1.c_str(),
                          MPI_MODE_CREATE | MPI_MODE_WRONLY | MPI_MODE_EXCL,
                          MPI_INFO_NULL, &si1fh),
            __LINE__, __FILE__);
  check_mpi(MPI_File_open(MPI_COMM_WORLD, sl1.c_str(),
                          MPI_MODE_CREATE | MPI_MODE_WRONLY | MPI_MODE_EXCL,
                          MPI_INFO_NULL, &sl1fh),
            __LINE__, __FILE__);
  check_mpi(MPI_File_open(MPI_COMM_WORLD, ss1.c_str(),
                          MPI_MODE_CREATE | MPI_MODE_WRONLY | MPI_MODE_EXCL,
                          MPI_INFO_NULL, &ss1fh),
            __LINE__, __FILE__);
  check_mpi(MPI_File_open(MPI_COMM_WORLD, si2.c_str(),
                          MPI_MODE_CREATE | MPI_MODE_WRONLY | MPI_MODE_EXCL,
                          MPI_INFO_NULL, &si2fh),
            __LINE__, __FILE__);
  check_mpi(MPI_File_open(MPI_COMM_WORLD, sl2.c_str(),
                          MPI_MODE_CREATE | MPI_MODE_WRONLY | MPI_MODE_EXCL,
                          MPI_INFO_NULL, &sl2fh),
            __LINE__, __FILE__);
  check_mpi(MPI_File_open(MPI_COMM_WORLD, ss2.c_str(),
                          MPI_MODE_CREATE | MPI_MODE_WRONLY | MPI_MODE_EXCL,
                          MPI_INFO_NULL, &ss2fh),
            __LINE__, __FILE__);
  check_mpi(MPI_File_open(MPI_COMM_WORLD, sim.c_str(),
                          MPI_MODE_CREATE | MPI_MODE_WRONLY | MPI_MODE_EXCL,
                          MPI_INFO_NULL, &simfh),
            __LINE__, __FILE__);
  check_mpi(MPI_File_open(MPI_COMM_WORLD, slm.c_str(),
                          MPI_MODE_CREATE | MPI_MODE_WRONLY | MPI_MODE_EXCL,
                          MPI_INFO_NULL, &slmfh),
            __LINE__, __FILE__);
  check_mpi(MPI_File_open(MPI_COMM_WORLD, ssm.c_str(),
                          MPI_MODE_CREATE | MPI_MODE_WRONLY | MPI_MODE_EXCL,
                          MPI_INFO_NULL, &ssmfh),
            __LINE__, __FILE__);
  ...
  if (rank == 0) {
    ...
    check_mpi(
        MPI_File_write_at(dimfh, 0, &buff, strlen(buff), MPI_CHAR, &status),
        __LINE__, __FILE__);
  }
  MPI_Barrier(MPI_COMM_WORLD);
  ...
  for (int i = 0; i < bpr; ++i) {
    ...
    if (rank == 0)
      printf("INFO   : reading (1/2) starting block %3d out of %3d\n", i + 1,
             bpr);
    ...
    char *rawdata = (char *)_mm_malloc(rawdata_n, 64);
    check_malloc(rawdata, __LINE__, __FILE__);

    // Gather sizes to determine common number of reads
    size_t rawdata_n_max = 0;
    check_mpi(MPI_Allreduce(&rawdata_n, &rawdata_n_max, 1,
                            MPI_UNSIGNED_LONG_LONG, MPI_MAX, MPI_COMM_WORLD),
              __LINE__, __FILE__);

    int NREADS = check_int_overflow(
        size_t(ceil(double(rawdata_n_max) / double(INT_MAX / 2))), __LINE__,
        __FILE__);
    if (NREADS <= 0) {
      if (rank == 0)
        printf("FATAL  : NREADS must be >= 1.");
      MPI_Abort(MPI_COMM_WORLD, 1);
    }
    ...
    // Read the bed data
    size_t bytes = 0;
    data.mpi_file_read_at_all<char *>(rawdata_n, offset, bedfh, MPI_CHAR,
                                      NREADS, rawdata, bytes);
  ...
  // Gather offsets
  // --------------
  size_t *AllN1 = (size_t *)_mm_malloc(nranks * sizeof(size_t), 64);
  check_malloc(AllN1, __LINE__, __FILE__);
  size_t *AllN2 = (size_t *)_mm_malloc(nranks * sizeof(size_t), 64);
  check_malloc(AllN2, __LINE__, __FILE__);
  size_t *AllNM = (size_t *)_mm_malloc(nranks * sizeof(size_t), 64);
  check_malloc(AllNM, __LINE__, __FILE__);

  check_mpi(MPI_Allgather(&rN1, 1, MPI_UNSIGNED_LONG_LONG, AllN1, 1,
                          MPI_UNSIGNED_LONG_LONG, MPI_COMM_WORLD),
            __LINE__, __FILE__);
  check_mpi(MPI_Allgather(&rN2, 1, MPI_UNSIGNED_LONG_LONG, AllN2, 1,
                          MPI_UNSIGNED_LONG_LONG, MPI_COMM_WORLD),
            __LINE__, __FILE__);
  check_mpi(MPI_Allgather(&rNM, 1, MPI_UNSIGNED_LONG_LONG, AllNM, 1,
                          MPI_UNSIGNED_LONG_LONG, MPI_COMM_WORLD),
            __LINE__, __FILE__);
  ...
  if (rank == 0)
    printf("INFO   : N1tot = %lu, N2tot = %lu, NMtot = %lu.\n", N1tot, N2tot,
           NMtot);
  ...
  for (int i = 0; i < bpr; ++i) {
    ...
    if (rank == 0)
      printf("INFO   : reading (2/2) starting block %3d out of %3d\n", i + 1,
             bpr);
    ...
    N1S = (size_t *)_mm_malloc(size_t(MLi) * sizeof(size_t), 64);
    check_malloc(N1S, __LINE__, __FILE__);
    N1L = (size_t *)_mm_malloc(size_t(MLi) * sizeof(size_t), 64);
    check_malloc(N1L, __LINE__, __FILE__);
    N2S = (size_t *)_mm_malloc(size_t(MLi) * sizeof(size_t), 64);
    check_malloc(N2S, __LINE__, __FILE__);
    N2L = (size_t *)_mm_malloc(size_t(MLi) * sizeof(size_t), 64);
    check_malloc(N2L, __LINE__, __FILE__);
    NMS = (size_t *)_mm_malloc(size_t(MLi) * sizeof(size_t), 64);
    check_malloc(NMS, __LINE__, __FILE__);
    NML = (size_t *)_mm_malloc(size_t(MLi) * sizeof(size_t), 64);
    check_malloc(NML, __LINE__, __FILE__);

    size_t N1 = 0, N2 = 0, NM = 0;
    data.sparse_data_get_sizes_from_raw(rawdata, uint(MLi), snpLenByt, 0, N1,
                                        N2, NM);
    // printf("DEBUG  : N1 = %15lu, N2 = %15lu, NM = %15lu\n", N1, N2, NM);

    // Alloc and build sparse structure
    uint *I1, *I2, *IM;
    I1 = (uint *)_mm_malloc(N1 * sizeof(uint), 64);
    check_malloc(I1, __LINE__, __FILE__);
    I2 = (uint *)_mm_malloc(N2 * sizeof(uint), 64);
    check_malloc(I2, __LINE__, __FILE__);
    IM = (uint *)_mm_malloc(NM * sizeof(uint), 64);
    check_malloc(IM, __LINE__, __FILE__);
    ...
    data.sparse_data_fill_indices(rawdata, MLi, snpLenByt, 0, N1S, N1L, I1, N2S,
                                  N2L, I2, NMS, NML, IM);
    ...
    check_mpi(MPI_Allreduce(&N1, &N1max, 1, MPI_UNSIGNED_LONG_LONG, MPI_MAX,
                            MPI_COMM_WORLD),
              __LINE__, __FILE__);
    check_mpi(MPI_Allreduce(&N2, &N2max, 1, MPI_UNSIGNED_LONG_LONG, MPI_MAX,
                            MPI_COMM_WORLD),
              __LINE__, __FILE__);
    check_mpi(MPI_Allreduce(&NM, &NMmax, 1, MPI_UNSIGNED_LONG_LONG, MPI_MAX,
                            MPI_COMM_WORLD),
              __LINE__, __FILE__);
    int MLimax = 0;
    check_mpi(
        MPI_Allreduce(&MLi, &MLimax, 1, MPI_INTEGER, MPI_MAX, MPI_COMM_WORLD),
        __LINE__, __FILE__);
    if (rank == 0)
      printf("INFO   : N1max = %lu, N2max = %lu, NMmax = %lu, MLimax = %d\n",
             N1max, N2max, NMmax, MLimax);

    int NREADS1 = check_int_overflow(
        size_t(ceil(double(N1max) / double(INT_MAX / 2))), __LINE__, __FILE__);
    int NREADS2 = check_int_overflow(
        size_t(ceil(double(N2max) / double(INT_MAX / 2))), __LINE__, __FILE__);
    int NREADSM = check_int_overflow(
        size_t(ceil(double(NMmax) / double(INT_MAX / 2))), __LINE__, __FILE__);
    int NREADSMLi = check_int_overflow(
        size_t(ceil(double(MLimax) / double(INT_MAX / 2))), __LINE__, __FILE__);
    if (rank == 0)
      printf(
          "INFO   : NREADS1 = %d, NREADS2 = %d, NREADSM = %d, NREADSMLi = %d\n",
          NREADS1, NREADS2, NREADSM, NREADSMLi);

    // Sparse Ones files
    offset = N1Off * sizeof(uint);
    data.mpi_file_write_at_all<uint *>(N1, offset, si1fh, MPI_UNSIGNED, NREADS1,
                                       I1);
    offset = size_t(MSi) * sizeof(size_t);
    data.mpi_file_write_at_all<size_t *>(
        size_t(MLi), offset, sl1fh, MPI_UNSIGNED_LONG_LONG, NREADSMLi, N1L);
    data.mpi_file_write_at_all<size_t *>(
        size_t(MLi), offset, ss1fh, MPI_UNSIGNED_LONG_LONG, NREADSMLi, N1S);

    // Sparse Twos files
    offset = N2Off * sizeof(uint);
    data.mpi_file_write_at_all<uint *>(N2, offset, si2fh, MPI_UNSIGNED, NREADS2,
                                       I2);
    offset = size_t(MSi) * sizeof(size_t);
    data.mpi_file_write_at_all<size_t *>(
        size_t(MLi), offset, sl2fh, MPI_UNSIGNED_LONG_LONG, NREADSMLi, N2L);
    data.mpi_file_write_at_all<size_t *>(
        size_t(MLi), offset, ss2fh, MPI_UNSIGNED_LONG_LONG, NREADSMLi, N2S);

    // Sparse Missing files
    offset = NMOff * sizeof(uint);
    data.mpi_file_write_at_all<uint *>(NM, offset, simfh, MPI_UNSIGNED, NREADSM,
                                       IM);
    offset = size_t(MSi) * sizeof(size_t);
    data.mpi_file_write_at_all<size_t *>(
        size_t(MLi), offset, slmfh, MPI_UNSIGNED_LONG_LONG, NREADSMLi, NML);
    data.mpi_file_write_at_all<size_t *>(
        size_t(MLi), offset, ssmfh, MPI_UNSIGNED_LONG_LONG, NREADSMLi, NMS);
  ...
  const auto du2 =
      std::chrono::duration_cast<std::chrono::milliseconds>(dt2).count();
  if (rank == 0)
    std::cout << "INFO   : time to convert the data: " << du2 / double(1000.0)
              << " seconds." << std::endl;
}
...
void BayesFH::mpi_assign_blocks_to_tasks(const uint numBlocks,
                                          const vector<int> blocksStarts,
                                          const vector<int> blocksEnds,
                                          const uint Mtot, const int nranks,
                                          const int rank, int *MrankS,
                                          int *MrankL, int &lmin, int &lmax) {

  if (numBlocks > 0) {

    if (nranks != numBlocks) {
      if (rank == 0) {
        printf("FATAL  : block definition does not match number of tasks (%d "
               "versus %d).\n",
               numBlocks, nranks);
    ...
    if (blocksStarts[0] != 1) {
      if (rank == 0) {
        printf("FATAL  : first marker in block definition file should be 1 but "
               "is %d\n",
               blocksStarts[0]);
    ...
    if (blocksEnds[numBlocks - 1] != Mtot) {
      if (rank == 0) {
        printf("FATAL  : last marker in block definition file should be Mtot = "
               "%d whereas is %d\n",
    ...
    // if (rank == 0)
    //    printf("INFO   : no marker block definition file used. Will go for
    //    even distribution over tasks.\n");
...
void BayesFH::init_from_scratch() { iteration_start = 0; }

void BayesFH::init_from_restart(const int K, const uint M, const uint Mtot,
                                 const uint Ntot, const int *MrankS,
                                 const int *MrankL,
                                 const bool use_xfiles_in_restart) {

  data.read_mcmc_output_csv_file(opt.mcmcOut, opt.save, K, sigmaG, sigmaE,
                                 estPi, iteration_restart);

  MPI_Barrier(MPI_COMM_WORLD);

  data.read_mcmc_output_bet_file(opt.mcmcOut, Mtot, iteration_restart, opt.thin,
                                 MrankS, MrankL, use_xfiles_in_restart, Beta);
...
#pragma omp parallel
  ...
  if (rank == 0)
    printf("INFO   : Full dataset includes Mtot=%d markers and Ntot=%d "
           "individuals.\n",
           Mtot, Ntot);

  // Handle marker groups: by default we assume a single group with all markers
  // belonging to it. In this case -S option is used for the mixture. Otherwise,
  // we expect a group file covering all markers with a group mixture file
  // covering all groups. Warning: passed mixture shall not contain the 0.0
  // value, it is added by default.
  ...
  // Display for control
  if (rank == 0)
    data.printGroupMixtureComponents();
  ...
  // Length of a marker in [byte] in BED representation (1 ind is 2 bits, so 1
  // byte is 4 inds)
  size_t snpLenByt = (Ntot % 4) ? Ntot / 4 + 1 : Ntot / 4;

  // Length of a marker in [uint] in BED representation (1 ind is 2 bits, so 1
  // uint is 16 inds)
  size_t snpLenUint = (Ntot % 16) ? Ntot / 16 + 1 : Ntot / 16;

  if (rank == 0)
    printf("INFO   : snpLenByt = %zu bytes; snpLenUint = %zu\n", snpLenByt,
           snpLenUint);

  // Define global marker indexing
  // -----------------------------
  int MrankS[nranks], MrankL[nranks], lmin = 1E9, lmax = 0;
  mpi_assign_blocks_to_tasks(data.numBlocks, data.blocksStarts, data.blocksEnds,
                             Mtot, nranks, rank, MrankS, MrankL, lmin, lmax);

  uint M = MrankL[rank];
  if (rank % 10 == 0) {
    printf(
        "INFO   : rank %3d will handle a block of %6d markers starting at %d\n",
  ...
  VectorXd denom(K - 1); // temporal variable for computing the inflation of the
                         // effect variance for a given non-zero component
  VectorXi m0(numGroups);    // non-zero elements per group
  cass.resize(numGroups, K); // variable storing the component assignment per
                             // group; EO RENAMING was v
  MatrixXi sum_cass(
      numGroups,
      K); // To store the sum of v elements over all ranks per group; was sum_v
  VectorXd Acum(M);
  VectorXb adaV(M); // Daniel adaptative scan Vector, ones will be sampled, 0
                    // will be set to 0
  ...
  // Adapt the --thin and --save options such that --save >= --thin and
  // --save%--thin = 0
  // ------------------------------------------------------------------------------------
  if (opt.save < opt.thin) {
    opt.save = opt.thin;
    if (rank == 0)
      printf("WARNING: opt.save was lower that opt.thin ; opt.save reset to "
             "opt.thin (%d)\n",
             opt.thin);
  }
  if (opt.save % opt.thin != 0) {
    ...
    if (rank == 0)
      printf("         opt.save reset to %d, the closest multiple of opt.thin "
             "(%d)\n",
             opt.save, opt.thin);
  }
  ...
  for (int i = 0; i < numGroups; i++) {
    ...
    priorPi.row(i).segment(1, km1) = priorPi(i, 0) *
                                     cVa.row(i).segment(1, km1).array() /
                                     cVa.row(i).segment(1, km1).sum();
  }
  ...
  if (opt.restart) {
    ...
    if (rank == 0)
      data.print_restart_banner(opt.mcmcOut.c_str(), iteration_restart,
                                iteration_start);
    ...
    xbetfp = opt.mcmcOut +
             ".xbet"; // Last saved iteration of bet; .bet has full history
  ...
  check_mpi(MPI_File_open(MPI_COMM_WORLD, outfp.c_str(),
                          MPI_MODE_CREATE | MPI_MODE_WRONLY | MPI_MODE_EXCL,
                          MPI_INFO_NULL, &outfh),
            __LINE__, __FILE__);
  check_mpi(MPI_File_open(MPI_COMM_WORLD, betfp.c_str(),
                          MPI_MODE_CREATE | MPI_MODE_WRONLY | MPI_MODE_EXCL,
                          MPI_INFO_NULL, &betfh),
            __LINE__, __FILE__);
  check_mpi(MPI_File_open(MPI_COMM_WORLD, xbetfp.c_str(),
                          MPI_MODE_CREATE | MPI_MODE_WRONLY | MPI_MODE_EXCL,
                          MPI_INFO_NULL, &xbetfh),
            __LINE__, __FILE__);
  check_mpi(MPI_File_open(MPI_COMM_WORLD, cpnfp.c_str(),
                          MPI_MODE_CREATE | MPI_MODE_WRONLY | MPI_MODE_EXCL,
                          MPI_INFO_NULL, &cpnfh),
            __LINE__, __FILE__);
  check_mpi(MPI_File_open(MPI_COMM_WORLD, xcpnfp.c_str(),
                          MPI_MODE_CREATE | MPI_MODE_WRONLY | MPI_MODE_EXCL,
                          MPI_INFO_NULL, &xcpnfh),
            __LINE__, __FILE__);
  check_mpi(MPI_File_open(MPI_COMM_WORLD, acufp.c_str(),
                          MPI_MODE_CREATE | MPI_MODE_WRONLY | MPI_MODE_EXCL,
                          MPI_INFO_NULL, &acufh),
            __LINE__, __FILE__);

  check_mpi(MPI_File_open(MPI_COMM_SELF, epsfp.c_str(),
                          MPI_MODE_CREATE | MPI_MODE_WRONLY | MPI_MODE_EXCL,
                          MPI_INFO_NULL, &epsfh),
            __LINE__, __FILE__);
  check_mpi(MPI_File_open(MPI_COMM_SELF, mrkfp.c_str(),
                          MPI_MODE_CREATE | MPI_MODE_WRONLY | MPI_MODE_EXCL,
                          MPI_INFO_NULL, &mrkfh),
            __LINE__, __FILE__);
  check_mpi(MPI_File_open(MPI_COMM_SELF, xivfp.c_str(),
                          MPI_MODE_CREATE | MPI_MODE_WRONLY | MPI_MODE_EXCL,
                          MPI_INFO_NULL, &xivfh),
            __LINE__, __FILE__);
  check_mpi(MPI_File_open(MPI_COMM_SELF, gamfp.c_str(),
                          MPI_MODE_CREATE | MPI_MODE_WRONLY | MPI_MODE_EXCL,
                          MPI_INFO_NULL, &gamfh),
            __LINE__, __FILE__);
  check_mpi(MPI_File_open(MPI_COMM_SELF, musfp.c_str(),
                          MPI_MODE_CREATE | MPI_MODE_WRONLY | MPI_MODE_EXCL,
                          MPI_INFO_NULL, &musfh),
            __LINE__, __FILE__);
  ...
  MPI_Offset offset = 0;
  if (rank == 0) {
    check_mpi(MPI_File_write_at(betfh, offset, &Mtot, 1, MPI_UNSIGNED, &status),
              __LINE__, __FILE__);
    check_mpi(
        MPI_File_write_at(xbetfh, offset, &Mtot, 1, MPI_UNSIGNED, &status),
        __LINE__, __FILE__);
    check_mpi(MPI_File_write_at(cpnfh, offset, &Mtot, 1, MPI_UNSIGNED, &status),
              __LINE__, __FILE__);
    check_mpi(
        MPI_File_write_at(xcpnfh, offset, &Mtot, 1, MPI_UNSIGNED, &status),
        __LINE__, __FILE__);
    check_mpi(MPI_File_write_at(acufh, offset, &Mtot, 1, MPI_UNSIGNED, &status),
              __LINE__, __FILE__);
  }
  ...
  if (opt.readFromBedFile && !opt.readFromSparseFiles) {
    ...
    data.load_data_from_bed_file(opt.bedFile, Ntot, M, rank, MrankS[rank], N1S,
                                 N1L, I1, N2S, N2L, I2, NMS, NML, IM,
                                 taskBytes);
    // for (int i=0; i<M; i++)
    //    printf("OUT BED bef rank %02d, marker %7d: 1,2,M  S = %6lu, %6lu,
    //    %6lu, L = %6lu, %6lu, %6lu\n", rank, i, N1S[i], N2S[i], NMS[i],
    //    N1L[i], N2L[i], NML[i]);
    ...
    data.load_data_from_sparse_files(rank, nranks, M, MrankS, MrankL, sparseOut,
                                     N1S, N1L, I1, N2S, N2L, I2, NMS, NML, IM,
                                     taskBytes);
    // for (int i=0; i<M; i++)
    //    printf("OUT SPARSE bef rank %02d, marker %7d: 1,2,M  S = %6lu, %6lu,
    //    %6lu, L = %6lu, %6lu, %6lu\n", rank, i, N1S[i], N2S[i], NMS[i],
    //    N1L[i], N2L[i], NML[i]);
    ...
    raw2, allele1, allele2, miss);
    }
    */

    data.load_data_from_mixed_representations(
        opt.bedFile, sparseOut, rank, nranks, Ntot, M, MrankS, MrankL, N1S, N1L,
        I1, N2S, N2L, I2, NMS, NML, IM, opt.threshold_fnz, USEBED, taskBytes);
    ...
    for (int i = 0; i < M; i++) {
      if (USEBED[i]) {
        ...
        data.sparse_data_get_sizes_from_raw(
            reinterpret_cast<char *>(&I1[N1S[i]]), 1, snpLenByt, data.numNAs,
            X1, X2, XM);
        // printf("data.sparse_data_get_sizes_from_raw => (%2d, %3d): X1 = %9lu,
        // X2 = %9lu, XM = %9lu < << <<<\n", rank, i, X1, X2, XM);
  ...
  // EO: BW cannot be evaluated properly in case of mixed representation as NA
  // corrections
  //    are applied in the loading routine
  //
  if (opt.mixedRepresentation) {
    printf("INFO   : rank %3d took %.3f seconds to load  %lu bytes and apply "
           "NA corrections to BED markers\n",
           rank, tl, taskBytes);
  } else {
    printf("INFO   : rank %3d took %.3f seconds to load  %lu bytes  =>  BW = "
           "%7.3f GB/s\n",
  ...
  if (opt.mixedRepresentation) {
    ...
    if (rank == 0)
      printf("INFO   : there is %d markers using BED representation out of %d "
             "(%6.2f%%)\n",
  ...
  // ----------------------------------------------------------
  if (rank == 0)
    printf("INFO   : BEFORE NA correction: Ntot = %d; snpLenByt = %zu [byte]; "
           "snpLenUint = %zu [uint]\n",
  ...
  if (data.numNAs > 0) {
    ...
    if (rank == 0)
      printf("INFO   : applying %d corrections to genotype data due to missing "
             "phenotype data (NAs)...\n",
             data.numNAs);
    ...
    if (rank == 0) {
      if (opt.mixedRepresentation) {
        printf("INFO   : ... finished applying NA corrections in %.2f seconds "
               "for SPARSE markers in mixed representation.\n",
               tna);
      } else {
        printf(
            "INFO   : ... finished applying NA corrections in %.2f seconds.\n",
            tna);
      }
    }
  ...
  if (rank == 0)
    printf("INFO   : AFTER  NA correction: Ntot = %d; snpLenByt = %zu [byte]; "
           "snpLenUint = %zu [uint]\n",
           Ntot, snpLenByt, snpLenUint);
  fflush(stdout);
  ...
  //
  for (int i = 0; i < M; i++) {
    if (USEBED[i]) { // Although N2L and NML do contain the info, I2 and IM were
                     // not allocated for BED markers! Done on the fly
      dalloc += (double)(snpLenUint * sizeof(uint)) * 1E-9;
    } else {
      dalloc +=
          (double)(N1L[i] + N2L[i] + NML[i]) * (double)sizeof(uint) * 1E-9;
    }
  }
  ...
  double *mave = (double *)_mm_malloc(size_t(M) * sizeof(double), 64);
  check_malloc(mave, __LINE__, __FILE__);
  double *mstd = (double *)_mm_malloc(size_t(M) * sizeof(double), 64);
  ...
  for (int i = 0; i < M; ++i) {
    ...
    tmp0 = double(Ntot - N1L[i] - N2L[i] - NML[i]) * (0.0 - mave[i]) *
           (0.0 - mave[i]);
  ...
  // previt_eps = (double*)malloc(NDB);  check_malloc(previt_eps, __LINE__,
  // __FILE__);
  ...
  // In case of restart we reset epsilon to last dumped state (sigmaE as well,
  // see init_from_restart)
  if (opt.restart) {
    for (int i = 0; i < Ntot; ++i)
    ...
    // EO: no need to broacast here, sigmaE is the same over all tasks as we
    // start from phenotype data EO: broadcast sigmaE from rank 0 to all the
    // others check_mpi(MPI_Bcast(&sigmaE, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD),
    // __LINE__, __FILE__);
  }
...
#ifdef NEWTONIAN
  SamplerGroupVar SamplerV(dist, Ntot, numGroups, A, B,
                           Init); // TODO input of A,B and Init
  SamplerEpsVar SamplerE(dist, Ntot, Aeps, Beps, InitEps);
#endif
  ...
  for (uint iteration = iteration_start; iteration < opt.chainLength;
    ...
    // if (replay_it) {
    //    printf("INFO: replay iteration with m0=%.0f sigG=%15.10f
    //    sigE=%15.10f\n", previt_m0, previt_sg, previt_se); m0        =
    //    previt_m0; sigmaG    = previt_sg; sigmaE    = previt_se; mu        =
    //    previt_mu; sync_rate = 0; for (int i=0; i<Ntot; ++i) epsilon[i] =
    //    previt_eps[i]; Beta      = previt_Beta; replay_it = false;
    //}
    ...
    for (int j = 0; j < lmax; j++) {
      ...
      // Sparse indices storage for BED markers needing expansion to SPARSE at
      // processing time
      //
	...
	//---------------------------------------
        ...
        if (adaV[j]) {
          ...
          if (USEBED[marker]) {
            ...
            //
            XI1 = (uint *)_mm_malloc(N1L[marker] * sizeof(uint), 64);
            check_malloc(XI1, __LINE__, __FILE__);
            XI2 = (uint *)_mm_malloc(N2L[marker] * sizeof(uint), 64);
            check_malloc(XI2, __LINE__, __FILE__);
            XIM = (uint *)_mm_malloc(NML[marker] * sizeof(uint), 64);
            check_malloc(XIM, __LINE__, __FILE__);

            size_t fake_n1s = 0, fake_n2s = 0, fake_nms = 0;
            size_t fake_n1l = 0, fake_n2l = 0, fake_nml = 0;

            // EO: bed data already ajusted for NAs
            data.sparse_data_fill_indices(
                reinterpret_cast<char *>(&I1[N1S[marker]]), 1, snpLenByt,
                data.numNAs, &fake_n1s, &fake_n1l, XI1, &fake_n2s, &fake_n2l,
                XI2, &fake_nms, &fake_nml, XIM);
            // printf("??? %lu %lu %lu\n", fake_n1l, fake_n2l, fake_nml);

            num = sparse_dotprod(epsilon, XI1, 0, N1L[marker], XI2, 0,
                                 N2L[marker], XIM, 0, NML[marker], mave[marker],
                                 mstd[marker], Ntot, marker);
          } else {

            num = sparse_dotprod(epsilon, I1, N1S[marker], N1L[marker], I2,
                                 N2S[marker], N2L[marker], IM, NMS[marker],
                                 NML[marker], mave[marker], mstd[marker], Ntot,
                                 marker);
          }

          // printf("it %2d, rank %2d, m %3d: num = %22.15f  USEBED=%d\n",
          // iteration, rank, marker, num, USEBED[marker]); continue;

          num += beta * double(Ntot - 1);
          // printf("it %d, rank %d, mark %d: num = %22.15f, %20.15f,
          // %20.15f\n", iteration, rank, marker, num, mave[marker],
          // mstd[marker]);
	  ...
	  //FHDT this changes in horseshoe, we use lambda_tilde
          for (int i = 1; i < 1 + km1; i++) {
            logL[i] = logL[i] -
	      0.5 * log((lambda_tilde/sigmaE) *dNm1  +
                                1.0) +
                      muk[i] * num * i_2sigE;
          }
          // logL.segment(1,km1) = logL.segment(1, km1).array()
          //- 0.5 * (sigG_E * dNm1 * cVa.row(groups[MrankS[rank] +
          // marker]).segment(1,km1).array() + 1.0).array().log()
          ...
          acum = 0.0;
          if (((logL.segment(1, km1).array() - logL[0]).abs().array() > 700.0)
                  .any()) {
            acum = 0.0;
          } else {
          ...
          for (int k = 0; k < K; k++) {
            if (p <= acum || k == km1) { // if we p is less than acum or if we
                                         // are already in the last mixt.
              if (k == 0) {
                Beta(marker) = 0.0;
              } else {
                Beta(marker) = dist.norm_rng(muk[k], sigmaE / denom[k - 1]);
                // printf("@B@ beta update %4d/%4d/%4d muk[%4d] = %15.10f with
                // p=%15.10f <= acum = %15.10f, denom = %15.10f, sigmaE =
                // %15.10f: beta = %15.10f\n", iteration, rank, marker, k,
                // muk[k], p, acum, denom[k-1], sigmaE, Beta(marker));
              }
              ...
              if (k + 1 >= K) {
                printf("FATAL  : iteration %d, marker = %d, p = %15.10f, acum "
                       "= %15.10f logL overflow with %d => %d\n",
                       iteration, marker, p, acum, k + 1, K);
                MPI_Abort(MPI_COMM_WORLD, 1);
              }
              if (((logL.segment(k + 1, K - (k + 1)).array() - logL[k + 1])
                       .abs()
                       .array() > 700.0)
                      .any()) {
	  ...
	  lambdaSampler.sampleLocalVar(10, tau, c, beta*beta );
        ...
        if (deltaBeta != 0.0) {
          ...
          // printf("it %d, task %3d, marker %5d has non-zero deltaBeta =
          // %15.10f (%15.10f, %15.10f) => %15.10f) 1,2,M: %lu, %lu, %lu\n",
          // iteration, rank, marker, deltaBeta, mave[marker], mstd[marker],
          // deltaBeta * mstd[marker], N1L[marker], N2L[marker], NML[marker]);
          ...
          if ((opt.bedSync || opt.sparseSync) && nranks > 1) {
            ...
            if (USEBED[marker]) {
              sparse_scaadd(deltaEps, deltaBeta, XI1, 0, N1L[marker], XI2, 0,
                            N2L[marker], XIM, 0, NML[marker], mave[marker],
                            mstd[marker], Ntot);
            } else {
              sparse_scaadd(deltaEps, deltaBeta, I1, N1S[marker], N1L[marker],
                            I2, N2S[marker], N2L[marker], IM, NMS[marker],
                            NML[marker], mave[marker], mstd[marker], Ntot);
            }
      ...
      // ------------------------------------------------------------
        ...
        // cout << "rank " << rank << " with M=" << M << " waiting for " << lmax
        // << endl;
        deltaBeta = 0.0;
      ...
      if (nranks > 1 && (sinceLastSync >= opt.syncRate || j == lmax - 1)) {
        ...
        check_mpi(MPI_Allreduce(&task_sum_abs_deltabeta, &cumSumDeltaBetas, 1,
      ...
      // printf("%d/%d/%d: deltaBeta = %20.15f = %10.7f - %10.7f; sumDeltaBetas
      // = %15.10f\n", iteration, rank, marker, deltaBeta, betaOld, beta,
      // cumSumDeltaBetas); fflush(stdout);

      if ((sinceLastSync >= opt.syncRate || j == lmax - 1) &&
        ...
        if (nranks > 1) {
          ...
          if (opt.bedSync) {
            ...
            // EO: check types below but should be fine as numbers should be
            // relatively small
            //
            ...
            double *task_stat =
                (double *)_mm_malloc(size_t(task_m2s) * 2 * sizeof(double), 64);
            ...
            check_mpi(MPI_Allgather(&task_m2s, 1, MPI_INT, tasks_len, 1,
            ...
            */
            ...
            char *task_bed = (char *)_mm_malloc(snpLenByt * task_m2s, 64);
            check_malloc(task_bed, __LINE__, __FILE__);
            ...
            for (int i = 0; i < mark2sync.size(); i++) {
              ...
              if (USEBED[m2si]) {
                ...
                memcpy(&task_bed[i * snpLenByt],
                       reinterpret_cast<char *>(&I1[N1S[m2si]]), snpLenByt);

              } else {
                // printf("rank %d will sync m %d => NEED TO CONVERT\n", rank,
                // m2si); cout << ">ntot = " << Ntot << endl;
                data.get_bed_marker_from_sparse(
                    &task_bed[(size_t)i * snpLenByt], Ntot, N1S[m2si],
                    N1L[m2si], &I1[N1S[m2si]], N2S[m2si], N2L[m2si],
                    &I2[N2S[m2si]], NMS[m2si], NML[m2si], &IM[NMS[m2si]]);
                // local check ;-)
                // size_t X1 = 0, X2 = 0, XM = 0;
                // data.sparse_data_get_sizes_from_raw(&task_bed[(size_t) i *
                // snpLenByt], 1, snpLenByt, data.numNAs, X1, X2, XM);
                // printf("data.sparse_data_get_sizes_from_raw => (%2d, %3d): X1
                // = %9lu, X2 = %9lu, XM = %9lu #?# vs %9lu %9lu %9lu\n", rank,
                // i, X1, X2, XM, N1L[m2si], N2L[m2si], NML[m2si]);
                // fflush(stdout);
              }
            }
            ...
            //
            char *glob_bed = (char *)_mm_malloc(snpLenByt * glob_m2s, 64);
            check_malloc(task_bed, __LINE__, __FILE__);

            check_mpi(MPI_Allgatherv(task_bed, tasks_len[rank], MPI_CHAR,
                                     glob_bed, tasks_len, tasks_dis, MPI_CHAR,
                                     MPI_COMM_WORLD),
                      __LINE__, __FILE__);

            double *glob_stats =
                (double *)_mm_malloc(size_t(glob_m2s * 2) * sizeof(double), 64);
            check_malloc(glob_stats, __LINE__, __FILE__);

            check_mpi(MPI_Allgatherv(task_stat, task_m2s * 2, MPI_DOUBLE,
                                     glob_stats, stats_len, stats_dis,
            ...
            for (int i = 0; i < glob_m2s; i++) {
              ...
              double lambda0 =
                  glob_stats[2 * i + 1] * (0.0 - glob_stats[2 * i]);
              // printf("rank %d, %2d lambda0 = %20.15f\n", rank, i, lambda0);
              // printf("rank %d lambda0 = %15.10f with mu = %15.10f, dbetsig =
              // %15.10f %d/%d\n", rank, lambda0, glob_stats[2 * i],
              // glob_stats[2
              // * i + 1], i, glob_m2s);

              // Get sizes from BED data
              // note: locally available from markers local to task but ignored
              // for now
              //
              size_t X1 = 0, X2 = 0, XM = 0;
              data.sparse_data_get_sizes_from_raw(
                  &glob_bed[(size_t)i * snpLenByt], 1, snpLenByt, data.numNAs,
                  X1, X2, XM);
              // printf("data.sparse_data_get_sizes_from_raw => (%2d, %3d): X1 =
              // %9lu, X2 = %9lu, XM = %9lu ###\n", rank, i, X1, X2, XM);
              // fflush(stdout);
              ...
              //
              uint *XI1 = (uint *)_mm_malloc(X1 * sizeof(uint), 64);
              check_malloc(XI1, __LINE__, __FILE__);
              uint *XI2 = (uint *)_mm_malloc(X2 * sizeof(uint), 64);
              check_malloc(XI2, __LINE__, __FILE__);
              uint *XIM = (uint *)_mm_malloc(XM * sizeof(uint), 64);
              check_malloc(XIM, __LINE__, __FILE__);
              ...
              data.sparse_data_fill_indices(
                  &glob_bed[(size_t)i * snpLenByt], 1, snpLenByt, data.numNAs,
                  &fake_n1s, &fake_n1l, XI1, &fake_n2s, &fake_n2l, XI2,
          ...
          // ----------------------
            ...
            // task_m2s, sinceLastSync); fflush(stdout);
            ...
            uint task_size = 0;
            for (int i = 0; i < task_m2s; i++) {
              task_size += (N1L[mark2sync[i]] + N2L[mark2sync[i]] +
                            NML[mark2sync[i]] + 3);
              task_stat[2 * i + 0] = mave[mark2sync[i]];
              task_stat[2 * i + 1] = mstd[mark2sync[i]] * dbet2sync[i];
              // printf("Task %3d, m2s %d/%d: 1: %8lu, 2: %8lu, m: %8lu, info:
              // 3); stats are (%15.10f, %15.10f)\n", rank, i, task_m2s, N1L[
              // mark2sync[i] ], N2L[ mark2sync[i] ], NML[ mark2sync[i] ],
              // task_stat[2 * i + 0], task_stat[2 * i + 1]);
            }
            // printf("Task %3d final task_size = %8d elements to send from
            // task_m2s = %d markers to sync.\n", rank, task_size, task_m2s);
            // fflush(stdout);

            // Get the total numbers of markers and corresponding indices to
            // gather
            ...
            check_mpi(MPI_Allgather(task_info, NEL, MPI_UNSIGNED, glob_info,
                                    NEL, MPI_UNSIGNED, MPI_COMM_WORLD),
                      __LINE__, __FILE__);
            ...
            for (int i = 0; i < nranks; i++) {
              ...
              stats_len[i] =
                  glob_info[2 * i] *
                  2; // number of markers in task i times 2 for stat1 and stat2
              stats_dis[i] = sdisp_;
              sdisp_ += glob_info[2 * i] * 2;
              glob_size += tasks_len[i];
              glob_m2s += glob_info[2 * i];
            }
            // printf("glob_info: markers to sync: %d, with glob_size = %7d
            // elements (sum of all task_size)\n", glob_m2s, glob_size);
            // fflush(stdout);

            // Build task's array to spread: | marker 1 | marker 2
            //                               | n1 | n2 | nm | data1 | data2 |
            //                               datam | n1 | n2 | nm | data1 | ...
            // -------------------------------------------------------------------------------------------------
            uint *task_dat =
                (uint *)_mm_malloc(size_t(task_size) * sizeof(uint), 64);
            ...
            for (int i = 0; i < task_m2s; i++) {
              ...
              if (USEBED[m2si]) {
                ...
                // EO TODO: keep sparse expansion from earlier on inbetween
                // syncronization

                uint *XI1 = (uint *)_mm_malloc(N1L[m2si] * sizeof(uint), 64);
                check_malloc(XI1, __LINE__, __FILE__);
                uint *XI2 = (uint *)_mm_malloc(N2L[m2si] * sizeof(uint), 64);
                check_malloc(XI2, __LINE__, __FILE__);
                uint *XIM = (uint *)_mm_malloc(NML[m2si] * sizeof(uint), 64);
                check_malloc(XIM, __LINE__, __FILE__);
                ...
                data.sparse_data_fill_indices(
                    reinterpret_cast<char *>(&I1[N1S[m2si]]), 1, snpLenByt,
                    data.numNAs, &fake_n1s, &fake_n1l, XI1, &fake_n2s,
                    &fake_n2l, XI2, &fake_nms, &fake_nml, XIM);

                for (uint ii = 0; ii < fake_n1l; ii++) {
                  task_dat[loc] = XI1[fake_n1s + ii];
                  loc += 1;
                }
                for (uint ii = 0; ii < fake_n2l; ii++) {
                  task_dat[loc] = XI2[fake_n2s + ii];
                  loc += 1;
                }
                for (uint ii = 0; ii < fake_nml; ii++) {
                  task_dat[loc] = XIM[fake_nms + ii];
                  loc += 1;
                }
                ...
                for (uint ii = 0; ii < N1L[m2si]; ii++) {
                  task_dat[loc] = I1[N1S[m2si] + ii];
                  loc += 1;
                }
                for (uint ii = 0; ii < N2L[m2si]; ii++) {
                  task_dat[loc] = I2[N2S[m2si] + ii];
                  loc += 1;
                }
                for (uint ii = 0; ii < NML[m2si]; ii++) {
                  task_dat[loc] = IM[NMS[m2si] + ii];
                  loc += 1;
                }
              }
            }
            ...
            //    printf("glob_size = %d\n", glob_size);
            uint *glob_dat =
                (uint *)_mm_malloc(size_t(glob_size) * sizeof(uint), 64);
            check_malloc(glob_dat, __LINE__, __FILE__);

            check_mpi(MPI_Allgatherv(task_dat, task_size, MPI_UNSIGNED,
                                     glob_dat, tasks_len, tasks_dis,
                                     MPI_UNSIGNED, MPI_COMM_WORLD),
                      __LINE__, __FILE__);
            _mm_free(task_dat);

            double *glob_stats = (double *)_mm_malloc(
                size_t(glob_size * 2) * sizeof(double), 64);
            check_malloc(glob_stats, __LINE__, __FILE__);
            ...
            for (int i = 0; i < glob_m2s; i++) {
              ...
              // loci, glob_dat[loci], glob_dat[loci + 1], glob_dat[loci + 2]);
              double lambda0 =
                  glob_stats[2 * i + 1] * (0.0 - glob_stats[2 * i]);
              // printf("rank %d lambda0 = %15.10f with mu = %15.10f, dbetsig =
              // %15.10f\n", rank, lambda0, glob_stats[2 * i], glob_stats[2 * i
              // + 1]);
              ...
              // M -> revert lambda 0 (so that equiv to add 0.0)
              size_t S =
                  loci + (size_t)(3 + glob_dat[loci] + glob_dat[loci + 1]);
              size_t L = glob_dat[loci + 2];
              // cout << "task " << rank << " M: start = " << S << ", len = " <<
              // L <<  endl;
              ...
              loci +=
                  3 + glob_dat[loci] + glob_dat[loci + 1] + glob_dat[loci + 2];
            }
            ...
            check_mpi(MPI_Allreduce(&dEpsSum[0], &deltaSum[0], Ntot, MPI_DOUBLE,
    ...
    // beta_squaredNorm); printf("==> after eps sync it %d, rank %d, epsilon[0]
    ...
    if (nranks > 1) {
      MPI_Barrier(MPI_COMM_WORLD);
      check_mpi(MPI_Allreduce(beta_squaredNorm.data(),
                              sum_beta_squaredNorm.data(),
                              beta_squaredNorm.size(), MPI_DOUBLE, MPI_SUM,
                              MPI_COMM_WORLD),
                __LINE__, __FILE__);
      check_mpi(MPI_Allreduce(cass.data(), sum_cass.data(), cass.size(),
    ...
    fflush(stdout);
    // TODO update sigmaG using Newtonian
    ...
    hypTau =
        dist.inv_gamma_rate_rng(0.5 + 0.5 * v0t, 1 / (tau0 * tau0) + 1 / tau);
...
//DT scaling for ishwaran rao
      // sigmaG[i] = SamplerV.sampleGroupVar(1,(double)m0[i]*
      // beta_squaredNorm[i],(double)m0[i],i);
    ...
    /*
      ...
      if (rank == 0) printf("INFO   : max sigmaG of iteration %d is %15.10f with
      sync rate of %d\n", iteration, max_sg, sync_rate);

      if (max_sg > 1.0) {
      if (sync_rate == 0) {
      if (rank == 0) {
      printf("CRITICAL: detected task with sigmaG = %15.10f and sync rate of
      %d\n", max_sg, sync_rate); printf("          => desperate situation,
      aborting...\n");
      }
      MPI_Abort(MPI_COMM_WORLD, 1);
      } else {
      ...
      //    printf("CRITICAL: divergence detected! Will cancel iteration and set
      up a lower sync rate\n");
      //}
      }
    */
    ...
    if (opt.covariates) {
      ...
      for (int i = 0; i < gamma_length; i++) {
        ...
        for (int k = 0; k < Ntot; k++) {
          num_f +=
              data.X(k, xI[i]) * (epsilon[k] + gamma_old * data.X(k, xI[i]));
        }
        ...
        for (int k = 0; k < Ntot; k++) {
          epsilon[k] =
              epsilon[k] + (gamma_old - gamma(xI[i])) * data.X(k, xI[i]);
          // cout << "adding " << (gamma_old - gamma(xI[i])) * data.X(k, xI[i])
          // << endl;
        }
      }
      // the next line should be uncommented if we want to use ridge for the
      // other cvoariates. sigmaF = inv_scaled_chisq_rng(0.001 + F,
      // (gamma.squaredNorm() + 0.001)/(0.001+F));
      sigmaF = s02F;
    }
    ...
    check_mpi(MPI_Bcast(&sigmaE, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD), __LINE__,
              __FILE__);

    // printf("sigmaE = %20.15f\n", sigmaE);

    double end_it = MPI_Wtime();
    // if (rank == 0) printf("TIME_IT: Iteration %5d on rank %4d took %10.3f
    // seconds\n", iteration, rank, end_it-start_it);

    // printf("%d epssqn = %15.10f %15.10f %15.10f %6d => %15.10f\n", iteration,
    // e_sqn, v0E, s02E, Ntot, sigmaE);
    if (rank % 10 == 0) {
      printf("RESULT : it %4d, rank %4d: proc = %9.3f s, sync = %9.3f (%9.3f + "
             "%9.3f), n_sync = %8d (%8d + %8d) (%7.3f / %7.3f), sigmaG = "
             "%15.10f, sigmaE = %15.10f, betasq = %15.10f, m0 = %10d\n",
             iteration, rank, end_it - start_it, it_sync_ar1 + it_sync_ar2,
             it_sync_ar1, it_sync_ar2, it_nsync_ar1 + it_nsync_ar2,
             it_nsync_ar1, it_nsync_ar2,
             (it_sync_ar1) / double(it_nsync_ar1) * 1000.0,
             (it_sync_ar2) / double(it_nsync_ar2) * 1000.0, sigmaG.sum(),
             sigmaE, beta_squaredNorm.sum(), m0.sum());
      fflush(stdout);
    }

    // cout<< "inv scaled parameters "<< v0G+m0 << "__"<<
    // (Beta.squaredNorm()*m0+v0G*s02G)/(v0G+m0) << endl; printf("inv scaled
    // parameters %20.15f __ %20.15f\n", v0G+m0,
    // (Beta.squaredNorm()*m0+v0G*s02G)/(v0G+m0)); sigmaE =
    // dist.inv_scaled_chisq_rng(v0E+Ntot,((epsilon).squaredNorm()+v0E*s02E)/(v0E+Ntot));
    // printf("sigmaG = %20.15f, sigmaE = %20.15f, e_sqn = %20.15f\n", sigmaG,
    // sigmaE, e_sqn); printf("it %6d, rank %3d: epsilon[0] = %15.10f, y[0] =
    // %15.10f, m0=%10.1f,  sigE=%15.10f,  sigG=%15.10f [%6d / %6d]\n",
    // iteration, rank, epsilon[0], y[0], m0, sigmaE, sigmaG, markerI[0],
    // markerI[M-1]);

    // Broadcast estPi from rank 0 (update moved up)
    check_mpi(
        MPI_Bcast(estPi.data(), estPi.size(), MPI_DOUBLE, 0, MPI_COMM_WORLD),
        __LINE__, __FILE__);
    ...
    if (iteration % opt.thin == 0) {
      ...
      if (rank == 0) {
        ...
        for (int jj = 0; jj < sigmaG.size(); ++jj) {
          cx = snprintf(&buff[strlen(buff)], LENBUF - strlen(buff), ", %20.15f",
                        sigmaG(jj));
          assert(cx >= 0 && cx < LENBUF - strlen(buff));
        }

        cx = snprintf(&buff[strlen(buff)], LENBUF - strlen(buff),
                      ", %20.15f, %20.15f, %7d, %4d, %2d", sigmaE,
                      sigmaG.sum() / (sigmaE + sigmaG.sum()), m0.sum(),
                      int(estPi.rows()), int(estPi.cols()));
        assert(cx >= 0 && cx < LENBUF - strlen(buff));
        ...
        check_mpi(MPI_File_write_at(outfh, offset, &buff, strlen(buff),
                                    MPI_CHAR, &status),
                  __LINE__, __FILE__);
      }
      ...
      if (rank == 0) {
        offset =
            sizeof(uint) + size_t(n_thinned_saved) *
                               (sizeof(uint) + size_t(Mtot) * sizeof(double));
        check_mpi(MPI_File_write_at(betfh, offset, &iteration, 1, MPI_UNSIGNED,
                                    &status),
                  __LINE__, __FILE__);
        check_mpi(MPI_File_write_at(acufh, offset, &iteration, 1, MPI_UNSIGNED,
                                    &status),
                  __LINE__, __FILE__);

        offset = sizeof(uint) + size_t(n_thinned_saved) *
                                    (sizeof(uint) + size_t(Mtot) * sizeof(int));
        check_mpi(MPI_File_write_at(cpnfh, offset, &iteration, 1, MPI_UNSIGNED,
                                    &status),
                  __LINE__, __FILE__);
      }

      offset = sizeof(uint) + sizeof(uint) +
               size_t(n_thinned_saved) *
                   (sizeof(uint) + size_t(Mtot) * sizeof(double)) +
               size_t(MrankS[rank]) * sizeof(double);
      check_mpi(MPI_File_write_at_all(betfh, offset, Beta.data(), M, MPI_DOUBLE,
                                      &status),
                __LINE__, __FILE__);
      check_mpi(MPI_File_write_at_all(acufh, offset, Acum.data(), M, MPI_DOUBLE,
                                      &status),
                __LINE__, __FILE__);
      ...
      check_mpi(MPI_File_write_at_all(cpnfh, offset, components.data(), M,
                                      MPI_INTEGER, &status),
                __LINE__, __FILE__);

      // EO: only iteration number and mu value (double) for the task-wise mus
      // files
      offset = size_t(n_thinned_saved) * (sizeof(uint) + sizeof(double));
      check_mpi(MPI_File_write_at(musfh, offset, &iteration, 1, MPI_UNSIGNED,
                                  &status),
                __LINE__, __FILE__);
      offset += sizeof(uint);
      check_mpi(MPI_File_write_at(musfh, offset, &mu, 1, MPI_DOUBLE, &status),
                __LINE__, __FILE__);
    ...
    if (iteration > 0 && iteration % opt.save == 0) {
      ...
      offset = 0;
      check_mpi(MPI_File_write_at(epsfh, offset, &iteration, 1, MPI_UNSIGNED,
                                  &status),
                __LINE__, __FILE__);
      check_mpi(MPI_File_write_at(mrkfh, offset, &iteration, 1, MPI_UNSIGNED,
                                  &status),
                __LINE__, __FILE__);
      if (opt.covariates) {
        check_mpi(MPI_File_write_at(gamfh, offset, &iteration, 1, MPI_UNSIGNED,
                                    &status),
                  __LINE__, __FILE__);
        check_mpi(MPI_File_write_at(xivfh, offset, &iteration, 1, MPI_UNSIGNED,
                                    &status),
                  __LINE__, __FILE__);
      }
      ...
      offset = sizeof(uint);
      check_mpi(
          MPI_File_write_at(epsfh, offset, &Ntot, 1, MPI_UNSIGNED, &status),
          __LINE__, __FILE__);
      check_mpi(MPI_File_write_at(mrkfh, offset, &M, 1, MPI_UNSIGNED, &status),
                __LINE__, __FILE__);
      check_mpi(MPI_File_write_at(xbetfh, offset, &iteration, 1, MPI_UNSIGNED,
                                  &status),
                __LINE__, __FILE__);
      check_mpi(MPI_File_write_at(xcpnfh, offset, &iteration, 1, MPI_UNSIGNED,
                                  &status),
                __LINE__, __FILE__);

      if (opt.covariates) {
        check_mpi(MPI_File_write_at(gamfh, offset, &gamma_length, 1,
                                    MPI_UNSIGNED, &status),
                  __LINE__, __FILE__);
        check_mpi(MPI_File_write_at(xivfh, offset, &gamma_length, 1,
                                    MPI_UNSIGNED, &status),
                  __LINE__, __FILE__);
      }
      ...
      offset = sizeof(uint) + sizeof(uint);
      check_mpi(
          MPI_File_write_at(epsfh, offset, epsilon, Ntot, MPI_DOUBLE, &status),
          __LINE__, __FILE__);
      check_mpi(MPI_File_write_at(mrkfh, offset, markerI.data(), markerI.size(),
                                  MPI_INT, &status),
                __LINE__, __FILE__);
      if (opt.covariates) {
        check_mpi(MPI_File_write_at(gamfh, offset, gamma.data(), gamma_length,
                                    MPI_DOUBLE, &status),
                  __LINE__, __FILE__);
        check_mpi(MPI_File_write_at(xivfh, offset, xI.data(), gamma_length,
                                    MPI_INT, &status),
                  __LINE__, __FILE__);
      }

      offset =
          sizeof(uint) + sizeof(uint) + size_t(MrankS[rank]) * sizeof(double);
      check_mpi(MPI_File_write_at_all(xbetfh, offset, Beta.data(), M,
                                      MPI_DOUBLE, &status),
                __LINE__, __FILE__);

      offset = sizeof(uint) + sizeof(uint) + size_t(MrankS[rank]) * sizeof(int);
      check_mpi(MPI_File_write_at_all(xcpnfh, offset, components.data(), M,
                                      MPI_INTEGER, &status),
                __LINE__, __FILE__);
      ...
      //    epsilon[Ntot-1]);
      //}
      // EO: to remove once MPI version fully validated; use the check_epsilon
      // utility to retrieve
      ...
      // iteration, rank, IrankS[rank], epsilon[IrankS[rank]],
      // IrankS[rank]+IrankL[rank]-1, epsilon[IrankS[rank]+IrankL[rank]-1]);
      ...
      if (rank == 0) {
        ...
        n = sprintf(tar, "dump_%s_%05d__%4d-%02d-%02d_%02d-%02d-%02d.tar",
                    opt.mcmcOutNam.c_str(), iteration, 1900 + ltm->tm_year,
                    1 + ltm->tm_mon, ltm->tm_mday, ltm->tm_hour, ltm->tm_min,
                    ltm->tm_sec);
        assert(n > 0);

        printf(
            "INFO   : will create tarball %s in %s with file listed in %s.\n",
            tar, opt.mcmcOutDir.c_str(), lstfp.c_str());
        // std::system(("ls " + opt.mcmcOut + ".*").c_str());
        // string cmd = "tar -czf " + opt.mcmcOutDir + "/tarballs/" + targz + "
        // -T " + lstfp;
        string cmd =
            "tar -cf " + opt.mcmcOutDir + "/tarballs/" + tar + " -T " + lstfp;
        // cout << "cmd >>" << cmd << "<<" << endl;
  ...
  const auto du3 =
      std::chrono::duration_cast<std::chrono::milliseconds>(dt3).count();
  if (rank == 0)
    printf("INFO   : rank %4d, time to process the data: %.3f sec, with %.3f "
           "(%.3f, %.3f) = %4.1f%% spent on allred (%d, %d)\n",
           rank, du3 / double(1000.0), tot_sync_ar1 + tot_sync_ar2,
           tot_sync_ar1, tot_sync_ar2,
...
//----------------------------------------------------------------------
int BayesFH::checkRamUsage() {

  int nranks;
  MPI_Comm_size(MPI_COMM_WORLD, &nranks);

  if (opt.checkRam && nranks != 1) {
    printf("#FATAL#: --check-RAM option runs only in single task mode "
           "(SIMULATION of --check-RAM-tasks with max "
           "--check-RAM-tasks-per-node tasks per node)!\n");
    MPI_Abort(MPI_COMM_WORLD, 1);
  }
  ...
  if (opt.checkRamTpn <= 0) {
    printf("#FATAL#: --check-RAM-tasks-per-node must be strictly positive! Was "
           "%d\n",
           opt.checkRamTpn);
    MPI_Abort(MPI_COMM_WORLD, 1);
  }
  ...
  // Alloc memory for sparse representation
  size_t *N1S, *N1L, *N2S, *N2L, *NMS, *NML;
  N1S = (size_t *)_mm_malloc(size_t(Mtot) * sizeof(size_t), 64);
  check_malloc(N1S, __LINE__, __FILE__);
  N1L = (size_t *)_mm_malloc(size_t(Mtot) * sizeof(size_t), 64);
  check_malloc(N1L, __LINE__, __FILE__);
  N2S = (size_t *)_mm_malloc(size_t(Mtot) * sizeof(size_t), 64);
  check_malloc(N2S, __LINE__, __FILE__);
  N2L = (size_t *)_mm_malloc(size_t(Mtot) * sizeof(size_t), 64);
  check_malloc(N2L, __LINE__, __FILE__);
  NMS = (size_t *)_mm_malloc(size_t(Mtot) * sizeof(size_t), 64);
  check_malloc(NMS, __LINE__, __FILE__);
  NML = (size_t *)_mm_malloc(size_t(Mtot) * sizeof(size_t), 64);
  check_malloc(NML, __LINE__, __FILE__);
  ...
  sl = sparseOut + ".sl1";
  check_mpi(MPI_File_open(MPI_COMM_WORLD, sl.c_str(), MPI_MODE_RDONLY,
                          MPI_INFO_NULL, &slfh),
            __LINE__, __FILE__);
  check_mpi(
      MPI_File_read_at_all(slfh, 0, N1L, Mtot, MPI_UNSIGNED_LONG_LONG, &status),
      __LINE__, __FILE__);
  ...
  check_mpi(MPI_File_open(MPI_COMM_WORLD, sl.c_str(), MPI_MODE_RDONLY,
            ...
            __LINE__, __FILE__);
  check_mpi(
      MPI_File_read_at_all(slfh, 0, N2L, Mtot, MPI_UNSIGNED_LONG_LONG, &status),
      __LINE__, __FILE__);
  check_mpi(MPI_File_close(&slfh), __LINE__, __FILE__);
  sl = sparseOut + ".slm";
  check_mpi(MPI_File_open(MPI_COMM_WORLD, sl.c_str(), MPI_MODE_RDONLY,
            ...
            __LINE__, __FILE__);
  check_mpi(
      MPI_File_read_at_all(slfh, 0, NML, Mtot, MPI_UNSIGNED_LONG_LONG, &status),
      __LINE__, __FILE__);
  check_mpi(MPI_File_close(&slfh), __LINE__, __FILE__);
  ...
  if (opt.markerBlocksFile != "")
    nranks = data.numBlocks;

  int nnodes = int(ceil(double(nranks) / double(tpn)));
  printf("INFO  : will simulate %d ranks on %d nodes with max %d tasks per "
         "node.\n",
         nranks, nnodes, tpn);

  int proctasks = 0;

  printf("Estimation RAM usage when dataset is processed with %2d nodes and "
         "%2d tasks per node\n",
         nnodes, tpn);
  ...
  for (int node = 0; node < nnodes; node++) {
    ...
    for (int i = 0; i < tpn; i++) {
      ...
      ramnode += GB;
      printf("   - t %3d  n %2d will attempt to alloc %.3f + %.3f + %.3f GB of "
             "RAM\n",
             i, node, n1 * sizeof(uint) * 1E-9, n2 * sizeof(uint) * 1E-9,
             nm * sizeof(uint) * 1E-9);

      allocs1[i] = (uint *)_mm_malloc(n1 * sizeof(uint), 64);
      check_malloc(allocs1[i], __LINE__, __FILE__);
      allocs2[i] = (uint *)_mm_malloc(n2 * sizeof(uint), 64);
      check_malloc(allocs2[i], __LINE__, __FILE__);
      allocsm[i] = (uint *)_mm_malloc(nm * sizeof(uint), 64);
      check_malloc(allocsm[i], __LINE__, __FILE__);

      printf("   - t %3d  n %2d sm %7d  l %6d markers. Number of 1s: %15lu, "
             "2s: %15lu, ms: %15lu => RAM: %7.3f GB; RAM on node: %7.3f with "
             "%d tasks\n",
             i, node, MrankS[node * tpn + i], MrankL[node * tpn + i], n1, n2,
             nm, GB, ramnode, tpn);
  ...
  if (proctasks != nranks) {
    printf("#FATAL#: Cannot fit %d tasks on %d nodes with %d x %d + %d x %d "
           "tasks per node! Ended up with %d.\n",
  ...
  printf("    => max RAM required on a node will be %7.3f GB on node %d\n", max,
         nodemax);
  printf("    => setting up your sbatch with %d tasks and %d tasks per node "
         "should work; Will require %d nodes!\n",
...
// Get directory and basename of bed file (passed with no extension via command
// line)
// ----------------------------------------------------------------------------------
string BayesFH::mpi_get_sparse_output_filebase(const int rank) {
  ...
  if (opt.sparseDir.length() > 0) {
    ...
    if (!S_ISDIR(stats.st_mode)) {
      if (rank == 0)
        printf("Fatal: requested directory for sparse output (%s) not found. "
               "Must be an existing directory (line %d in %s).\n",
...
uint BayesFH::set_Ntot(const int rank) {

  uint Ntot = opt.numberIndividuals;

  if (Ntot == 0) {
    printf("FATAL  : opt.numberIndividuals is zero! Set it via "
           "--number-individuals in call.");
    MPI_Abort(MPI_COMM_WORLD, 1);
  }

  if (Ntot != data.numInds - data.numNAs) {
    if (rank == 0)
      printf("WARNING: opt.numberIndividuals set to %d but will be adjusted to "
             "%d - %d = %d due to NAs in phenotype file.\n",
...
uint BayesFH::set_Mtot(const int rank) {

  uint Mtot = opt.numberMarkers;

  if (Mtot == 0)
    throw("FATAL  : opt.numberMarkers is zero! Set it via --number-markers in "
          "call.");
  ...
  if (opt.markerBlocksFile != "" && opt.numberMarkers > 0) {
    opt.numberMarkers = 0;
    if (rank == 0)
      printf("WARNING: --number-markers option ignored, a marker block "
             "definition file was passed!\n");
  }
