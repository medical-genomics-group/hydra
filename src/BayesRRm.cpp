...
void BayesRRm::offset_vector_f64(double *__restrict__ vec, const double offset,
                                 const int N) {
...
void BayesRRm::set_vector_f64(double *__restrict__ vec, const double val,
                              const int N) {
...
void BayesRRm::copy_vector_f64(double *__restrict__ dest,
...
double BayesRRm::sum_vector_elements_f64_base(const double *__restrict__ vec,
                                              const int N) {
...
double BayesRRm::sum_vector_elements_f64(const double *__restrict__ vec,
                                         const int N) {
...
void BayesRRm::sum_vectors_f64(double *__restrict__ out,
...
// inline
void BayesRRm::sparse_add(double *__restrict__ vec, const double val,
                          const uint *__restrict__ IX, const size_t NXS,
                          const size_t NXL) {
...
void BayesRRm::sparse_scaadd(double *__restrict__ vout, const double dMULT,
...
// -----------------------------------------------------
void BayesRRm::mpi_define_blocks_of_markers(const int Mtot, int *MrankS,
...
// -------------------------------------------------------------------------
void BayesRRm::check_whole_array_was_set(const uint *array, const size_t size,
...
void BayesRRm::mpi_assign_blocks_to_tasks(const uint numBlocks,
...
void BayesRRm::init_from_scratch() { iteration_start = 0; }

void BayesRRm::init_from_restart(const int K, const uint M, const uint Mtot,
    ...
    for (int j = 0; j < lmax; j++) {
      ...
      if (j < M) {
        ...
        if (adaV[j]) {
          ...
          for (int i = 1; i <= km1; ++i) {
            denom(i - 1) =
                dNm1 + sigE_G * cVaI(groups[MrankS[rank] + marker], i);
          ...
          for (int i = 1; i < 1 + km1; i++) {
            logL[i] = logL[i] -
                      0.5 * log(sigG_E * dNm1 *
                                    cVa(groups[MrankS[rank] + marker], i) +
                                1.0) +
                      muk[i] * num * i_2sigE;
          }
