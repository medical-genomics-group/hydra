...
void LinPred::predict_genetic_values(string outfile) {
    ...
    uint block_size = block_rows_a * I;
    double *buff_a = (double *) malloc(sizeof(double) * elem_per_proc_a);
    double *buff_c = (double *) malloc(sizeof(double) * block_size);
    ...
    for (uint i = 0; i < block_rows_a; i++) {
        for (uint j = 0; j < I; j++) {
            for (uint k = 0; k < M; k++) {
                buff_c[i * I + j] += buff_a[i * M + k] * b[j * M + k];
            }
        }
    }
    // gather results
    ...
    MPI_Gather(buff_c, block_size, MPI_DOUBLE, c,
