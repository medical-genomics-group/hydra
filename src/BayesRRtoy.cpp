        ...
        for(int j=0; j < M; ++j) {
            ...
            if (marker%100 == 0)
                printf("  -> marker %6i has mean %13.6f on %ld elements [%13.6f, %13.6f]  Sq. Norm = %13.6f, Var = %9.7f\n",
                       marker,
                       double(data.Z.col(marker).mean()),
                       data.Z.col(marker).size(),
                       double(data.Z.col(marker).minCoeff()),
                       double(data.Z.col(marker).maxCoeff()),
                       double(data.ZPZdiag[marker]),
                       double(data.Z.col(marker).squaredNorm()) / double(data.Z.col(marker).size()));
        }
    }
